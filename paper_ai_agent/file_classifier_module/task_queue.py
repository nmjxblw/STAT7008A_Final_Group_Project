...
class TaskQueueManager(metaclass=SingletonMeta):
    ...
    def add_task(self, file_path: str) -> bool:
        ...
        try:
            ...
            logger.info(
                f"任务已添加到队列：{path.name}（队列长度：{self._task_queue.qsize()}）"
            )
            return True
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        Returns:
            dict: 包含队列状态的字典
                ...
                - is_paused: 工作线程是否处于暂停状态
                - queue_size: 队列中待处理的任务数
                - total_tasks: 总任务数
                - completed_tasks: 已完成任务数
                - failed_tasks: 失败任务数
                - current_task: 当前正在处理的任务（文件路径）
                - pending_tasks: 待处理任务数（包括队列中的和正在处理的）
        """
        with self._lock:
            return {
    ...
    def _worker_loop(self):
        ...
        while not self._stop_event.is_set():
            try:
                ...
                try:
                    ...
                    logger.info(
                        f"任务处理成功：{Path(file_path).name}（已完成：{self._completed_tasks}/{self._total_tasks}）"
                    )
                except Exception as e:
        ...
        if remaining > 0:
            logger.info(
                f"工作线程停止信号收到，还有{remaining}个任务待处理，继续完成..."
            )
...
def add_classify_task(file_path: PathLike) -> bool:
    """
    添加文档归类任务到队列（异步）
